...
const TrendingNotesGallery: React.FC<TrendingNotesGalleryProps> = ({ onSelect, initialTab = 'recommend' }) => {
  ...
  useEffect(() => {
    // Listen for global task completion events
    const handleTaskComplete = (event: CustomEvent) => {
        const { type, status } = event.detail;
        if (type === 'SCRAPE_TRENDS' && status === 'COMPLETED') {
            // If we are currently viewing the category that was just scraped (or 'recommend'), refresh
            // Actually, we don't know the category from the event easily without payload.
            // But usually refreshing the current list is safe.
            if (isMounted.current) {
                console.log('Received task completion event, refreshing notes...');
                fetchNotes(1);
            }
        }
    };

    window.addEventListener('TASK_COMPLETED', handleTaskComplete as EventListener);
    return () => {
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      if (taskId) {
          ...
          // Poll for completion
          // Use a ref to control polling so we can stop it if the global event fires
          // Actually, since we now rely on global event, we can remove the aggressive polling
          // Or keep it as a fallback. Let's keep it simple: Wait for event OR poll slowly.
          
          let attempts = 0;
          let taskStatus = 'PENDING';
          
          // Fallback polling (in case event is missed or socket fails)
          while (attempts < 60) { // Max 2 mins
              await new Promise(r => setTimeout(r, 2000));
              
              // If component unmounted or we are no longer refreshing (handled by event), stop
              ...
              try {
                  ...
                  if (taskStatus === 'COMPLETED') {
                      // Handled by global event listener usually, but double check here
                      // If notes are still empty, fetch again
                      if (notes.length === 0) {
    ...
    <div className="mt-6">
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto items-center">
            ...
            <div className="flex bg-gray-100 p-1 rounded-md">
                <button
                    ...
                    className={`px-3 py-1 text-xs rounded-md transition-colors ${selectedType === 'all' ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
                >
                    全部
                </button>
                <button
                    onClick={() => setSelectedType('video')}
                    className={`px-3 py-1 text-xs rounded-md transition-colors ${selectedType === 'video' ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
                >
                    视频
                </button>
                <button
                    onClick={() => setSelectedType('image')}
                    className={`px-3 py-1 text-xs rounded-md transition-colors ${selectedType === 'image' ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
                >
                    图文
                </button>
            </div>
      ...
      {showAnalysisModal && currentAnalysis && (
        <NoteAnalysisModal
            ...
            onSelect={(note) => {
                if (onSelect) {
                    onSelect({ ...note, analysis_result: currentAnalysis });
                } else {
                    // Default behavior: Navigate to generate page with remix structure
                    navigate('/generate', { 
                        state: { 
                            topic: note.title, // Use title as initial topic
                            remix_structure: {
                                hook_type: currentAnalysis.hook_type,
                                hook_analysis: currentAnalysis.hook_analysis,
                                structure_breakdown: currentAnalysis.structure_breakdown,
                                cta_strategy: currentAnalysis.cta_strategy,
